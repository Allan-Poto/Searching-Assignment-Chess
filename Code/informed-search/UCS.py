...
def search(rows, cols, grid, enemy_pieces, own_pieces, goals):
    ...
    frontier.append( Node(Game_state.get_piece_in_play().get_position(), None, [Game_state.get_piece_in_play().get_position()], 0, 1) ) # Initialized position
    while len(frontier) > 0:
        current_node = heapq.heappop(frontier) # Remove the first node in the queue
        ...
        if Game_state.isGoalState(current_node.get_state()):
            ...
            for action in Game_state.generateActions(current_node.get_state()):
                if action in explored:
                    ...
                    heapq.heappush( frontier, Node(action, current_node, path, path_cost, depth) )
    return result, 0
