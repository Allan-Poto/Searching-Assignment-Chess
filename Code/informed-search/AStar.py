...
#############################################################################
def apply_heuristics(curr_path_cost, curr_coordinate, goals, heuristics="Euclidean"):
    if heuristics == "Euclidean": # Closest Goal Euclidean Distance
        h_n = 10000000000
        for goal in goals:
            # euclidean distance rounded down using int() 
            distance = int(math.sqrt( (curr_coordinate[0] - goal[0])**2 + (curr_coordinate[1] - goal[1])**2 ))
            if distance < h_n:
                h_n = distance
        f_cost = curr_path_cost + h_n
    else:
        # heuristics of 0
        f_cost = curr_path_cost
    return f_cost
...
def search(rows, cols, grid, enemy_pieces, own_pieces, goals):
    ...
    initial_fn = apply_heuristics(0, Game_state.get_piece_in_play().get_position(), Game_state.get_goals())
    # Initialized starting position node with the heuristics cost
    frontier.append( [ initial_fn ,Node(Game_state.get_piece_in_play().get_position(), None, [Game_state.get_piece_in_play().get_position()], 0, 1) ] )
    while len(frontier) > 0:
        fn, current_node = heapq.heappop(frontier) # Remove the first node in the queue, fn is the path cost based on heuristics
        if current_node.get_state() in explored: # Checking if we have already visited that node
            # Compare the current cost to reach that node to the previous time reaching that node 
            if current_node.get_path_cost() < gHatBest[ current_node.get_state() ][0]:
                gHatBest[ current_node.get_state() ] = [current_node.get_path_cost(), current_node.get_path()]
            else: # If the previous way of reaching that node is cheaper, can drop this path 
                continue
        else:
            gHatBest[ current_node.get_state() ] = [current_node.get_path_cost(), current_node.get_path()] # Track the cost of reaching that node 
        ...
        if Game_state.isGoalState(current_node.get_state()):
            ...
            for action in Game_state.generateActions(current_node.get_state()):
                if action in explored:
                    ...
                    path = current_node.get_path() + [action] # Update the path with the node explored
                    path_cost = current_node.get_path_cost() + Game_state.get_board().get_coordinate_value(action[0], action[1])
                    depth = current_node.get_depth() + 1
                    fn = apply_heuristics(path_cost, action, Game_state.get_goals())
                    heapq.heappush( frontier, [fn, Node(action, current_node, path, path_cost, depth)] )
    return result, 0
